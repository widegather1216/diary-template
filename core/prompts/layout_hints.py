LAYOUT_HINTS = {
    "mandalart": {
        ...
        "text": "    - [Mandalart]: MUST be an 81-cell nested grid! To prevent layout calculations from freezing or crashing the server's layout engine, you MUST use HTML `<table>` elements instead of Flexbox or CSS Grid for this complex 9x9 layout. Create a main 3x3 outer table (class=\"outer-table\"). Inside every single cell of this outer table (class=\"outer-cell\"), place a nested 3x3 inner table (class=\"inner-table\"). Set `border-collapse: collapse;` on all tables. Style the outer table and inner tables with `width: 100%; height: 100%;`. Set `border: 2px solid <color>;` on the outer cells (class=\"outer-cell\") to clearly distinguish the nine 3x3 regions. Set `border: 1px solid <color>;` on the inner cells (class=\"inner-cell\") for the 81 individual boxes, where <color> matches the style theme (e.g., `#ccc` or `#e5e7eb` for Cute/Minimal, or `#333` for guide mode). Use nested `<repeat count=\"3\">` tags to generate the 3 rows (`<tr>`) of both the outer and inner tables cleanly. Do NOT use any Flexbox inside the cells for layout, as it will crash WeasyPrint."
    },
    "monthly": {
