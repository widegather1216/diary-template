LAYOUT_HINTS = {
    "mandalart": {
        ...
        "text": "    - [Mandalart / 만다라트]: MUST be a single page containing ONLY the header (Title and optional Metadata like Date/Goal) and a large 9x9 grid (total 81 cells). Do NOT include any other major sections like a lined Notes area or checklists, as the grid must occupy almost the entire page. To prevent WeasyPrint from freezing or crashing, you MUST use a single HTML `<table>` element with 9 rows (`<tr>`) and 9 cells (`<td>`) per row, styled with `border-collapse: collapse; width: 100%; table-layout: fixed;`. Inside the table, use `<repeat count=\"9\">` for rows and `<repeat count=\"9\">` for cells. To prevent WeasyPrint from collapsing the table height, you MUST style the cells explicitly with a height of 85px to 95px (e.g., `.mandalart-table td { height: 90px; }`), making the total table height 765px to 855px. Give cells a default border of `1px solid <color>`. To visually separate the nine 3x3 sub-grids, you MUST apply a thicker border (e.g., `3px solid <color>`) to the right border of the 3rd and 6th cells (`td:nth-child(3), td:nth-child(6)`) and the bottom border of the cells in the 3rd and 6th rows (`tr:nth-child(3) td, tr:nth-child(6) td`), where <color> matches the style theme. Do NOT use Flexbox inside the table cells."
    },
    "monthly": {
