LAYOUT_HINTS = {
    "mandalart": {
        ...
        "text": "    - [Mandalart]: MUST be an 81-cell nested grid! Create a main 3x3 flex grid (3 rows, 3 cells per row). Inside EVERY single one of the 9 main cells, create ANOTHER nested 3x3 grid (3 sub-rows, 3 sub-cells per sub-row). You MUST use nested `<repeat count=\"3\">` tags to generate this outer-and-inner 3x3x3x3 layout cleanly (total 81 cells). To prevent layout collapsing in WeasyPrint, you MUST style the main cell container (class=\"cell\") with `display: flex; flex-direction: column;` and the inner sub-grid (class=\"sub-grid\") with `flex-grow: 1; width: 100%;` (never use `height: 100%`). The sub-rows (class=\"sub-row\") MUST have `display: flex; flex-direction: row; width: 100%; flex: 1;`. The sub-cells (class=\"sub-cell\") MUST have `flex: 0 0 33.333%; width: 33.333%; box-sizing: border-box;`. To ensure all inner grid lines are visible, the sub-grid container (class=\"sub-grid\") MUST have `border-top: 1px solid <color>; border-left: 1px solid <color>;` and sub-cells (class=\"sub-cell\") MUST have `border-right: 1px solid <color>; border-bottom: 1px solid <color>;` where <color> is a thin, lighter color (e.g., `#ccc` or `#e5e7eb` for Cute/Minimal, or `#333` for guide mode) and matches the sub-cell and sub-grid border colors exactly. NEVER remove or hide borders on any edge sub-cells (do NOT use `:last-child` or `:nth-child` resets on sub-cells)."
    },
    "monthly": {
