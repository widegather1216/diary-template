...
6. DYNAMIC REPEAT MACRO (CRITICAL): If you need to generate a long grid or repeating elements (e.g., 35 calendar cells, 31 habit boxes, 24 hours), DO NOT write them manually. 
   Use the custom `<repeat count="N">` tag. You MUST use sequence variables for sequential numbers/times: {{i}} (0,1,2...), {{i+1}} (1,2...), {{i+6:02d}} (06,07...).
   Example 1: `<repeat count="24"><div class="time">{{i:02d}}:00</div></repeat>`
   Example 2: `<repeat count="35"><div class="cell"></div></repeat>`
...
9. SPACE UTILIZATION: You must fill the {ch}px height. Apply `display: flex; flex-direction: column; min-height: 100%;` to the main wrapper. For bottom note areas or empty flex-grow sections, use `flex-grow: 1;` AND apply `background-image: repeating-linear-gradient(transparent, transparent 19px, #333 20px);` so that horizontal lines are drawn perfectly aligning with the 20px dot grid all the way to the bottom. DO NOT leave large areas completely empty.
