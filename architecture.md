...
| :--- | :--- |
| **`app.py`** | 클라이언트의 웹 요청을 접수하고 비동기 PDF 생성, 즉시 파기, 다운로드 게이트웨이 역할을 수행하며 라이프사이클을 전담 제어. |
...
### 2.2. 생성 엔진 (`core/generator.py` & `core/prompts/` 패키지)
...
* **공백 무시(Space-insensitive) 키워드 매칭**: 사용자가 입력한 제목과 설명을 소문자화 및 공백을 모두 제거한 상태로 변환한 뒤, `layout_hints.py`에 등록된 키워드 사전과 대조하여 일치하는 가이드(예: 가계부, 식단표 등)를 추가 매칭하여 프롬프트에 주입합니다.
...
| **4** | 문자 밑줄 금지 | 문자 `_______`를 사용하는 방식 금지. Flexbox 컨테이너의 하단 테두리(`border-bottom`) 활용 |
...
| **7 (7a~7h)** | 테두리 폐쇄 및 2px 방지 | 부모는 상/좌만, 자식은 하/우 테두리만 주어 2px 겹침을 방지하되 외곽선은 빈틈없이 닫음 |
| **7h** | 메모 상단 닫기 규칙 | 메모 영역(`.lined-bg`)의 상단에 경계선이 없거나 스페이스가 뜰 때 상단 테두리를 강제 보완 |
